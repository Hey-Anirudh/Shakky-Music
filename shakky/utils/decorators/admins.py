...
def AdminActual(mystic):
    async def wrapper(client, message):
        ...
        if message.from_user.id not in SUDOERS:
            try:
                member_info = await app.get_chat_member(message.chat.id, message.from_user.id)
                member = member_info.privileges
            except:
                return
            if not member or not getattr(member, 'can_manage_video_chats', False):
...
def ActualAdminCB(mystic):
    async def wrapper(client, CallbackQuery):
        ...
        if not is_non_admin:
            try:
                member_info = await app.get_chat_member(CallbackQuery.message.chat.id, CallbackQuery.from_user.id)
                a = member_info.privileges
            except:
                return await CallbackQuery.answer(_["general_4"], show_alert=True)
            if not a or not getattr(a, 'can_manage_video_chats', False):
                if CallbackQuery.from_user.id not in SUDOERS:
